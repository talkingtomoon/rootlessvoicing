...
이조 후 **최저음이 MIDI 48–59 (C3–B3)** 에 들어오도록 전체를 옥타브 단위로 이동 (`placement.ts`).
이게 canonical 배치이고 암기 모드 채점 기준(옥타브까지 일치해야 정답).
진행을 순서대로 재생할 때는 `placeProgression`이 두 번째 코드부터 ±1 옥타브 보정으로
보이스리딩을 유지한다(경계 키에서 canonical 배치끼리는 옥타브 점프가 생길 수 있음 — 재생용만 보정).
